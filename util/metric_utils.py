...
def calc_all_datasets(prd_root: str, tar_root: str, test_256: bool = False, save_path: str = './'):
    ...
    for key in dataset.keys():
        ...
        if key == 'UIEB VAL' or key == 'EUVP':
            tar_path = os.path.join(tar_root, dataset[key], 'gt')
            double_metrics = calc_multi_double_img_metrics(tmp_path, tar_path)
        if len(double_metrics) == 0:
            res = single_metrics
        else:
            res = {**single_metrics, **double_metrics}
        record_metric(res, os.path.join(save_path, prd_root.split('\\')[-1].split('_')[1] + '.xlsx'),
                      key)


if __name__ == '__main__':
    tar_root = r'C:\Users\Ira\Desktop\test_dateset\test_dateset'
    prd_root = r'E:\BaiduNetdiskDownload\new_predict\new_predict\2017_IBLA_TIP'
    calc_all_datasets(prd_root, tar_root, test_256=False)